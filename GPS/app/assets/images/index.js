/* eslint import/no-unresolved: 0 */

const logo = require('./logo.png');
const backArrow = require('./back_btn.png');
const rightArrow = require('./drop_down.png');
const leftBlueArrow = require('./left_blue_arrow.png');
const editOder = require('./edit_order.png');
const account = require('./account.png');
const earn = require('./earn.png');
const home = require('./home.png');
const logout = require('./logout.png');
const notifi = require('./notifi.png');
const order = require('./order.png');
const review = require('./review.png');
const menu = require('./menu.png');
const grayRightArrow = require('./gray_right_arrow.png');
const noImage = require('./no_image.png');
const callIcon = require('./call.png');
const locationIcon = require('./locat.png');
const addIcon = require('./add.png');
const unselectIcon = require('./unselect_icon.png');
const selectedIcon = require('./selected_icon.png');

const images = {
  logo,
  backArrow,
  rightArrow,
  leftBlueArrow,
  editOder,
  account,
  earn,
  home,
  logout,
  notifi,
  order,
  review,
  menu,
  grayRightArrow,
  noImage,
  callIcon,
  locationIcon,
  addIcon,
  unselectIcon,
  selectedIcon,
};

export default images;
